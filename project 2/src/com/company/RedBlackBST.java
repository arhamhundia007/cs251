...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    public Node delete_recursively(Node node, Key key) {
    ...
    public Node next_largest(Node node){
        if (node.left != null){
    ...
    //keeps going to the left through red nodes
    //this is used because when the tree has reached the node which it has to delete,
    //it will set the right node to that minimum value (the one found by next_largest - the code is very similar
    //and then balance the subtree
    //and will keep balancing until it reaches the node again.
    public Node delete_min(Node node){
        if (node.left == null){
            return null;
        }
